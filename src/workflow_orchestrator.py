...
class ODGWorkflowOrchestrator:
    ...
    def process_odg_pdf(self, pdf_path: str, 
                       enable_scraping: bool = True,
                       enable_synthesis: bool = True,
                       enable_notion_sync: bool = True) -> WorkflowResult:
        """
        ...
        try:
            ...
            # Step 2: Scrape decreto status (if enabled)
            if enable_scraping:
                self.logger.info("Step 2: Scraping decreto status")
            ...
            if enable_synthesis:
                self.logger.info("Step 3: Generating AI synthesis")
                deliberations = self._generate_synthesis(
                    deliberations, metrics, errors
                )
            
            # Step 4: Sync to Notion (if enabled)
            if enable_notion_sync:
                self.logger.info("Step 4: Syncing to Notion")
                deliberations = self._sync_to_notion(
                    deliberations, session_info, metrics, errors
                )
            ...
            success = (metrics.errors == 0 and 
                      metrics.synced_to_notion == metrics.total_deliberations)
    ...
    def _scrape_decreto_status(self, deliberations: List[Dict], 
                              session_info: Dict,
                              metrics: WorkflowMetrics,
    ...
    def _generate_synthesis(self, deliberations: List[Dict],
                           metrics: WorkflowMetrics,
                           errors: List[str]) -> List[Dict]:
        ...
        for deliberation in deliberations:
            try:
                ...
                self.logger.debug(f"Synthesis generated for deliberation {deliberation.get('numero', 'N/A')}")
    ...
    def _sync_to_notion(self, deliberations: List[Dict],
                       session_info: Dict,
                       metrics: WorkflowMetrics,
                       errors: List[str]) -> List[Dict]:
    ...
    def _create_backup(self, deliberations: List[Dict],
                      session_info: Dict,
                      metrics: WorkflowMetrics) -> str:
        ...
        try:
            ...
            if deliberations:
                with open(csv_path, 'w', newline='', encoding='utf-8') as f:
                    fieldnames = ['seduta', 'numero', 'tipo_atto', 'oggetto', 'proponente',
                                'fs_flag', 'pubblicato', 'url_decreto', 'sintesi_rapida']
    ...
    def generate_detailed_synthesis(self, deliberation_id: str,
                                  synthesis_type: SynthesisType = SynthesisType.EXECUTIVE) -> Dict[str, Any]:
        """
