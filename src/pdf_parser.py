...
class ODGPDFParser:
    ...
    def setup_logging(self, log_level: str) -> None:
        ...
        logging.basicConfig(
            ...
            handlers=[logging.StreamHandler(), logging.FileHandler("logs/pdf_parser.log")],
        )
    ...
    def _extract_session_info(self, text: str) -> Dict:
        ...
        session_info = {"numero_seduta": None, "data_seduta": None, "anno": None}

        # Extract session number
        session_match = re.search(self.session_patterns["numero_seduta"], text, re.IGNORECASE)
        if not session_match:
            session_match = re.search(
                self.session_patterns["numero_seduta_alt"], text, re.IGNORECASE
            )
        if session_match:
    ...
    def _extract_deliberations(self, text: str) -> List[Dict]:
        ...
        for match in matches:
            ...
            if session_info.get("data_seduta"):
                deliberation["data_seduta"] = session_info["data_seduta"]
    ...
    def _extract_oggetto(self, section: str) -> Optional[str]:
        ...
        cleaned_section = re.sub(
            r"^\d+\.\s*(?:DGR|DCR|DECRETO|ORDINANZA|CIRCOLARE)\s*", "", section, flags=re.IGNORECASE
        )
        ...
        for line in lines:
            ...
            if len(line) > 20 and not re.match(
                r"^\s*(?:Proponente|PROPONENTE|Assessore)", line, re.IGNORECASE
            ):
                # Clean up the line
