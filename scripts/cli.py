...
class ODGCLI:
    ...
    def load_environment(self):
        ...
        if not self.notion_token or not self.notion_database_id:
            print("⚠️  Warning: NOTION_TOKEN and NOTION_DATABASE_ID not set. Some commands may fail.")
    ...
    def command_process(self, args):
        ...
        try:
            ...
            if args.verbose:
                ...
                print(f"  Synthesized: {metrics.synthesized_successfully}/{metrics.parsed_successfully}")
                print(f"  Synced to Notion: {metrics.synced_to_notion}/{metrics.synthesized_successfully}")
            ...
            if args.detailed:
                ...
                if self.notion_token and self.notion_database_id:
                    try:
                        ...
                        stats = notion.get_sync_statistics()
                        self.print_success(f"Notion: Database accessible")
                ...
                if self.anthropic_api_key:
                    try:
                        ...
                        self.print_success(f"AI Synthesizer: Working")
...
Examples:
  ...
  %(prog)s test-connection --detailed
        ...
        # Process command
        process_parser = subparsers.add_parser('process', help='Process ODG PDF through complete workflow')
        process_parser.add_argument('--pdf', required=True, help='Path to ODG PDF file')
        process_parser.add_argument('--no-scraping', action='store_true', help='Skip decreto scraping')
        ...
        verify_parser.add_argument('--days', type=int, default=30, help='Days to check back (default: 30)')
        ...
        dashboard_parser.add_argument('--source', choices=['backup', 'notion'], default='backup', help='Data source')
        dashboard_parser.add_argument('--backup-dir', default='data/backups', help='Backup directory')
        ...
        synthesis_parser.add_argument('--numbers', nargs='+', type=int, required=True, help='Deliberation numbers')
        synthesis_parser.add_argument('--type', choices=['executive', 'esecutiva', 'operational', 'operativa', 'communicative', 'comunicativa'], 
                                    default='executive', help='Synthesis type')
        
        # Test connection command
        test_parser = subparsers.add_parser('test-connection', help='Test connection to all services')
        test_parser.add_argument('--detailed', action='store_true', help='Run detailed connection tests')
